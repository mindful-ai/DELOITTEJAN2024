...
sc = SparkContext("local[2]", "Text File Stream - Stock Data Case Study")
...
# Using spark streaming context, we can create a DStream that represents text file streaming data and the source will be text files created in the specified directory. As and when a new file is created the contents of the text file will be streamed into our application. More details are available under Basic Sources at: https://spark.apache.org/docs/2.1.1/streaming-programming-guide.html#input-dstreams-and-receivers

# In[ ]:


# inStream = ssc.socketTextStream("localhost", 9999)
inStream = ssc.textFileStream('/tmp/streamdir/')
